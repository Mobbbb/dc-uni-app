let $Live2D = null
let $Live2DModelWebGL = null
let $Live2DMotion = null
let MotionQueueManager = null
let AMotion = null
...
;(function() {
	...
	aa._$62 = function(aQ, aU) {
		try {
			...
			if (aN > ay._$T7) {
				aQ._$NP |= aa._$4s;
				var aR = ay._$T7;
				var aI = "_$gi _$C _$li , _$n0 _$_ version _$li ( SDK : " + aR + " < _$f0 : " + aN +
					" )@_$SS#loadModel()\n";
				throw new J(aI);
			}
			var aL = aS._$nP();
	...
	u.prototype.update = function(aI, aL) {
		if (this._$iP == 0) {
			...
			this._$Fo = (Math.sqrt((this.p1.x - this.p2.x) * (this.p1.x - this.p2.x) + (this.p1.y - this.p2.y) *
				(this.p1.y - this.p2.y)));
			return;
		}
	...
	u.prototype._$oo = function(aN, aI) {
		...
		var aO = (Math.sqrt((this.p1.x - this.p2.x) * (this.p1.x - this.p2.x) + (this.p1.y - this.p2.y) * (this
			.p1.y - this.p2.y)));
				...
				if (a1 == 2) {
					...
					if (a1 == 3) {
						...
						if (a1 == 4) {
							...
							var a5 = a9[2];
							var a3 = a9[3];
							return (1 - a3) * ((1 - a5) * ((1 - a7) * (aT + (aS - aT) * a8) + a7 * (bs + (br -
								bs) * a8)) + a5 * ((1 - a7) * (aN + (aM - aN) * a8) + a7 * (bl + (bk -
								bl) * a8))) + a3 * ((1 - a5) * ((1 - a7) * (bd + (bb - bd) * a8) + a7 * (
								aX + (aW - aX) * a8)) + a5 * ((1 - a7) * (a6 + (a4 - a6) * a8) + a7 * (
								aR + (aQ - aR) * a8)));
						} else {
							var aV = 1 << a1;
							var aY = new Float32Array(aV);
			...
			if (aN == 1) {
				...
				if (aN == 2) {
					...
					if (aN == 3) {
						...
						var bO = bZ * b1 * b3;
						for (var bt = 0; bt < aJ;) {
							a3[aQ] = b8 * ba[bt] + b7 * a9[bt] + bU * aP[bt] + bS * aO[bt] + b6 * a6[bt] + b5 *
								a4[bt] + bQ * aL[bt] + bO * aK[bt];
							++bt;
							a3[aQ + 1] = b8 * ba[bt] + b7 * a9[bt] + bU * aP[bt] + bS * aO[bt] + b6 * a6[bt] +
								b5 * a4[bt] + bQ * aL[bt] + bO * aK[bt];
							++bt;
							aQ += bH;
						}
					} else {
						if (aN == 4) {
							...
							var aT = bY * bZ * b1 * b3;
							for (var bt = 0; bt < aJ;) {
								a3[aQ] = bk * bD[bt] + bi * bB[bt] + aW * bo[bt] + aV * bm[bt] + bc * by[bt] +
									bb * bx[bt] + aS * be[bt] + aR * bd[bt] + bs * bG[bt] + br * bE[bt] + a1 *
									bv[bt] + a0 * bu[bt] + bh * bA[bt] + bf * bz[bt] + aU * bn[bt] + aT * bl[
									bt];
								++bt;
								a3[aQ + 1] = bk * bD[bt] + bi * bB[bt] + aW * bo[bt] + aV * bm[bt] + bc * by[
									bt] + bb * bx[bt] + aS * be[bt] + aR * bd[bt] + bs * bG[bt] + br * bE[bt] +
									a1 * bv[bt] + a0 * bu[bt] + bh * bA[bt] + bf * bz[bt] + aU * bn[bt] + aT *
									bl[bt];
								++bt;
								aQ += bH;
							}
				...
				if (a2 == 2) {
					...
					if (a2 == 3) {
						...
						if (a2 == 4) {
							...
							bm._$Yr._$fL = (1 - a4) * ((1 - a6) * (bC + (bB - bC) * a8) + a6 * (bz + (by - bz) *
								a8)) + a4 * ((1 - a6) * (bv + (bt - bv) * a8) + a6 * (br + (bq - br) * a8));
							...
							bm._$Yr._$gL = (1 - a4) * ((1 - a6) * (bC + (bB - bC) * a8) + a6 * (bz + (by - bz) *
								a8)) + a4 * ((1 - a6) * (bv + (bt - bv) * a8) + a6 * (br + (bq - br) * a8));
							...
							bm._$Yr._$B0 = (1 - a4) * ((1 - a6) * (bC + (bB - bC) * a8) + a6 * (bz + (by - bz) *
								a8)) + a4 * ((1 - a6) * (bv + (bt - bv) * a8) + a6 * (br + (bq - br) * a8));
							...
							bm._$Yr._$z0 = (1 - a4) * ((1 - a6) * (bC + (bB - bC) * a8) + a6 * (bz + (by - bz) *
								a8)) + a4 * ((1 - a6) * (bv + (bt - bv) * a8) + a6 * (br + (bq - br) * a8));
							...
							bm._$Yr._$qT = (1 - a4) * ((1 - a6) * (bC + (bB - bC) * a8) + a6 * (bz + (by - bz) *
								a8)) + a4 * ((1 - a6) * (bv + (bt - bv) * a8) + a6 * (br + (bq - br) * a8));
						} else {
	...
	v._$er = function(aM) {
		...
		var aH = ["../_$_r/_$t0/_$Ri/_$_P.512/_$CP._$1", "../_$_r/_$t0/_$Ri/_$_P.512/_$vP._$1",
	...
	E.transformPoints_sdk2 = function(a0, bc, a5, aP, aI, aR, aQ, aU) {
		...
		for (var ba = aP; ba < aW; ba += aI) {
			...
			if (bd < 0 || a7 < 0 || aQ <= bd || aU <= a7) {
				...
				if (!aZ) {
					aZ = true;
					aT = 0.25 * (aR[((0) + (0) * a1) * 2] + aR[((aQ) + (0) * a1) * 2] + aR[((0) + (aU) * a1) *
						2] + aR[((aQ) + (aU) * a1) * 2]);
					aS = 0.25 * (aR[((0) + (0) * a1) * 2 + 1] + aR[((aQ) + (0) * a1) * 2 + 1] + aR[((0) + (aU) *
						a1) * 2 + 1] + aR[((aQ) + (aU) * a1) * 2 + 1]);
				...
				if (bn + bm < 1) {
					...
					bc[ba] = aR[aV + 2 * (aQ + 1) + 2] * (bn - 1 + bm) + aR[aV + 2 * (aQ + 1)] * (1 - bn) + aR[
						aV + 2] * (1 - bm);
					bc[ba + 1] = aR[aV + 2 * (aQ + 1) + 3] * (bn - 1 + bm) + aR[aV + 2 * (aQ + 1) + 1] * (1 -
						bn) + aR[aV + 3] * (1 - bm);
				}
			}
		}
	...
	E.prototype.transformPoints_sdk1 = function(aJ, aR, aL, a0, aU, aP, aZ) {
		...
		for (var aK = aP; aK < aI; aK += aZ) {
			...
			if (aX + aW < 1) {
				...
				a0[aK] = aT[aV + 2 * (aM + 1) + 2] * (aX - 1 + aW) + aT[aV + 2 * (aM + 1)] * (1 - aX) + aT[aV +
					2] * (1 - aW);
				a0[aK + 1] = aT[aV + 2 * (aM + 1) + 3] * (aX - 1 + aW) + aT[aV + 2 * (aM + 1) + 1] * (1 - aX) +
					aT[aV + 3] * (1 - aW);
			}
		}
	};
	...
	C.prototype._$ZT = function() {
		...
		if (this.firstDraw) {
			...
			this.anisotropyExt = aH.getExtension("EXT_texture_filter_anisotropic") || aH.getExtension(
				"WEBKIT_EXT_texture_filter_anisotropic") || aH.getExtension(
	...
	C.prototype._$Uo = function(aS, aT, aL, aU, aV, aN, aM, aO) {
		...
		if (this.clipBufPre_clipContextMask != null) {
			...
			a0.uniform4f(this.u_baseColor_Loc, aI.x * 2 - 1, aI.y * 2 - 1, aI._$EL() * 2 - 1, aI._$5T() * 2 -
			1);
			...
			if (a1) {
				...
				a0.uniformMatrix4fv(this.u_clipMatrix_Loc_Off, false, this.getClipBufPre_clipContextDraw()
					.matrixForDraw);
	...
	aC._$NS = (180 / Math.PI);
	aC.PI_F = Math.PI;
	aC._$kT = [0, 0.012368, 0.024734, 0.037097, 0.049454, 0.061803, 0.074143, 0.086471, 0.098786, 0.111087, 0.12337,
	...
	r._$W2 = 0;
	r.SYSTEM_INFO = null;
	r.USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36';
	r.isIPhone = function() {
		if (!r.SYSTEM_INFO) {
			r.setup();
		}
...
function L2DBaseModel() {
    ...
    this.alpha           = 1;
    this.accAlpha        = 0;
    this.lipSync         = false; // 銉儍銉椼偡銉炽偗銇屾湁鍔广亱銇┿亞銇�
    this.lipSyncValue    = 0;     // 鍩烘湰銇�0锝�1
    this.accelX          = 0;
    this.accelY          = 0;
    this.accelZ          = 0;
    this.dragX           = 0;
...
{
    this.EPSILON         = 0.01; // 澶夊寲銇渶灏忓€わ紙銇撱伄鍊や互涓嬨伅鐒¤銇曘倢銈嬶級
    this.faceTargetX     = 0;
    this.faceTargetY     = 0;
    this.faceX           = 0;
    this.faceY           = 0;
...
PlatformManager.prototype.loadTexture = function(model/*ALive2DModel*/, no/*int*/, path/*String*/, callback) {
    ...
    loadedImage.onload = function() {
        ...
        var texture = gl.createTexture();	 // 銉嗐偗銈广儊銉ｃ偑銉栥偢銈с偗銉堛倰浣滄垚銇欍倠
        if (!texture){ console.error("Failed to generate gl texture name."); return -1; }

        if(model.isPremultipliedAlpha() == false){
            // 涔楃畻娓堛偄銉儠銈°儐銈偣銉併儯浠ュ銇牬鍚�
            gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL, 1);
        }
        gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, 1);	// image銈掍笂涓嬪弽杌�
        ...
        // 鐢诲儚銇嬨倝WebGL銉嗐偗銈广儊銉ｅ寲銈掔敓鎴愩仐銆併儮銉囥儷銇櫥閷�
        model.setTexture(no, texture);// 銉€儑銉伀銉嗐偗銈广儊銉ｃ倰銈汇儍銉�
        
        // 銉嗐偗銈广儊銉ｃ偑銉栥偢銈с偗銉堛倰瑙ｆ斁
        texture = null;
...
//    ArrayBuffer 銇嬨倝 JSON 銇鎻涖仚銈�
//============================================================
PlatformManager.prototype.jsonParseFromBytes = function(buf){
    
    var jsonStr;
    
    // BOM銇湁鐒°伀蹇溿仒銇﹀嚘鐞嗐倰鍒嗐亼銈�
    // UTF-8銇瓸OM銇�0xEF 0xBB 0xBF锛�10閫叉暟锛�239 187 191锛�
...
export const Live2D = $Live2D
export const Live2DModelWebGL = $Live2DModelWebGL
export const Live2DMotion = $Live2DMotion
